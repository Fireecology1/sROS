...
For the PPDF fires, we sought to incorporate the effects of understory grass and herbaceous curing effects on fire behaviour [@agee2006; @youngblood2008]. This involved normalizing all observations, following [@cheney1995], to a nearly fully cured grass condition (95 % C) in order to estimate near-peak seasonal spread rate. Although the grass biomass in these stands only represented a fraction (\~3 %) of the total available surface fuel, grass curing is has been observed by operational practitioners to have a strong influence on ROS in these stands [@beck2003]. We assumed that understory herbaceous curing would be 50 % as influential compared to a true grassland, and calculated new 95% C estimated ROS (*ROS'*) values for these plots, as follows:
...
#  geom_point(data=ppdf, aes(size=SFC), shape=21) +
  ...
  stat_smooth(data=s.fires.con, method=lm, 
              formula=y~x-1, se=FALSE, colour='gray') + #linear fit
#  stat_function(fun=asym, colour='deepskyblue4', linewidth=1) + #asymtote?
  stat_smooth(data=s.fires4 %>% filter(ISI>=10), method=lm, 
              formula=y~x, se=FALSE, colour='deepskyblue3') +
...
asym2 <- lm(data=s.fires4 %>% filter(ISI>=10), ROS~ISI)
asym2f <- function(isi) {
  predict(asym2, newdata=list(ISI=isi))
}
...
### [slope asymtote trials]

```{r label=slope asymtote, echo=FALSE}

#make asymtote as linear function
#For aggregate 'level off' (CR) model, Should cross ROS=20 at ISI=65
#very similar to current sf.crisi.fun(65)
#combine that with a shallow slope, say 0.35
...
#sf.crisi.con<- nls(data=s.fires.mod, formula= 
#               ROS ~ a.value * (1-exp(-b*ISI))^c, 
#               start=list(b=0.1, c=3))
# a=25, b=0.032563, c=1.836628

# New CR equation with slope asymptote
sf.agg.asy<- nls(data=s.fires.mod, formula= 
               ROS ~ (0.15 * ISI + 10) * (1-exp(-b*ISI))^c, 
               start=list(b=0.1, c=3))

sf.asy.fun <- function(isi) {
  predict(sf.agg.asy, newdata=list(ISI=isi))
}

ag <- ggplot(s.fires4, aes(x=ISI, y=ROS)) + 
  geom_point() + 
  stat_function(fun=sf.asy.fun, xlim=c(0, 70)) + 
  stat_function(fun=sf.crisi.fun, xlim=c(0, 70)) +
  stat_function(fun=asym3, xlim=c(0, 70))
