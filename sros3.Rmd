...
Simple inspection of our fire database [@perrakis2023] revealed certain obvious patterns. Among `r nrow(fd.all)-nrow(d1)` fires in conifer fuel types (surface and crown), a clear, though noisy linear relationship is evident between ROS and wind speed, separated by type of fire behaviour (Fig. 2).
...
In terms of fuel type, the lone surface fire observation in black spruce was grouped with five observations in mixed jack pine- black spruce in order to separate spruce-containing observations (S) from the pine-type fires. The influence of conifer fuel type (FT) was only significant when combined with *ISI^2^* or *ISI~sa~^2^* in the boreal conifer fires. In addition to 6 fires in spruce, there were 19 fires in jack pine (JP), and 26 in stands dominated by other pine species (OP). Although FT was significant, the only significant contrast (Tukey HSD: $\alpha=0.05$) was between OP and JP, with OP fires exhibiting slightly slower ROS compared with JP fires (Fig. 3). FT was not a significant predictor in the aggregate data models.
...
Figure 4 shows the surface ROS observations plotted by overall fuel type (boreal conifer, deciduous, or PPDF) and SFC against *ISI~sa~*, along with three of the fitted models: M9 (calculated at SFC=1.5 kg m^-2^), M11, and M14 (boreal conifer only). PPDF fires adjusted to 95% curing conditions were used in model fitting, while the original observations (not included in models) are also shown. Fuel type (Con, PPDF, or Decid) was not significant in any form of model using *ISI~sa~*.
...
The present surface fire models involve mainly weather and moisture-related inputs, with some influence from fuel type and SFC in some models. While the inclusion of SFC in Models 7 and 9 offers a helpful second predictor for ROS that can be estimated from fuel loading and indices of moisture deficit [@degroot2009], the influence of SFC was highest at low *ISI* (or *ISI~sa~*) values, and became negligible under higher ISI conditions (Table S2).
