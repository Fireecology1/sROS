...
#function
sf.powersa <- function(ws, mcsa=10) {
...
mc.power2 <- 13
mc.power3 <- 17
ROS.powersa.points <- sf.powersa(
  ws=wind.range, mcsa=mc.power) #ROS using power model
ROS.powersa.points2 <- sf.powersa( #for low range
  ws=wind.range2, mcsa=mc.power2)
ROS.powersa.points3 <- sf.powersa(
...
power.df <- data.frame(ROS=ROS.powersa.points,
                       isi.m=isim.powersa.points)

power2.df <- data.frame(ROS=ROS.powersa.points2, 
                        isi.m=isim.powersa.points2)

power3.df <- data.frame(ROS=ROS.powersa.points3,
...
The best models based on the evaluation diagnostics varied depending on dataset (conifer vs. aggregated) and the range of input variables. Simple and relatively high performing models were obtained using the *ISI^2* alone, though with much better performance (higher ER2, lower MAE, AIC and other error measures) using the conifer (m3) than the aggregated (m6) datasets. Non-linear model forms using Eq. 1 or 4 were unable to automatically fit the asymptote parameters. Visual inspection produced asymptotes (*m*=0.15, *i~agg~*=10, *i~con~*=13) that fitted fairly high-performing models for the aggregated and conifer datasets, respectively (m10-11, m12-13). The best performance overall across the range of data was achieved by the  models with separate wind and litter moisture inputs (e.g. m16).

For the aggregated dataset models (6-18), performance improved when SFC or FT were included as predictors compared with using only ISI or ws and mc.  

[cut all this; we then estimated the parameters of an oblique asymptote (Eq. [4]) based on visual inspection and an assumed gradual ROS rise beyond the range of data to account for some extrapolation. While symptotes for conifer and aggregated datasets were slightly different, similar in slope but translated slightly higher for conifer models compared with aggregate data models; aggregate: *m*=0.15, *i*=10; conifer: *m*=0.15, *i*=13. The remaining parameters were fitted to the data, producing some of the best models for the aggregated forest data, particularly with *ISIsa* (m11) or when FT (m16) was included: RMSE \< 1.5, ER2 ~ 0.6, MAE ~1 m min^-1^). The best aggregated d 
