...
lp.mods3 <- leaps::regsubsets(ROS ~ ., 
                             data=s.fires.squared, #force.out='DMC', 
                             nbest=4, nvmax=5, intercept=FALSE)
...
lp.mods.lst <- summary(lp.mods2)[[7]] 
#uses lp.mods2 from the 'Complex modelling attempts' section

#following Perplexity code
wh <- summary(lp.mods2)$which  #generates rows=models, cols=(intercept, preds)

wh.df0 <- as.data.frame(wh) %>%
...
wh.models <- wh.df %>%
  ...
  ungroup() %>%
  filter(mod_size <= 4, model_rank <=3) # keep only sizes up to 4, best 3
