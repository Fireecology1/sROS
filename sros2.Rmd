...
mod.list <- mod.list.long[1:(length(mod.list)-3)]
...
acc.tab3 <- mutate(acc.tab2, 
                   Model=modnames[1:(length(modnames)-3)], 
...
names(pred.list) <- modnames[1:(length(modnames)-3)]
...
v13vals <- data.frame(v1, v2, v3) %>% as.matrix %>% t() %>%
  as.data.frame()

predAdd.df <- data.frame(Model=modnames[11:13], 
                         ISI.5=v13vals[,1] %>% round(2),
                         ISI.10=v13vals[,2] %>% round(2),
                         ISI.15=v13vals[,3] %>% round(2))

pred3.df <- rbind(pred2.df, predAdd.df)

acc.tab4 <- left_join(pred3.df, acc.tab3, by='Model') %>%
  select(Model, nobs, AIC, efron.r2, everything())
