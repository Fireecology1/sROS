...
#Import data frame
fd <- read.csv('./files/fire_data_apr2024.csv')  # with corrected SFC from McRae, Sharp p#16 to sharp.th
...
Fig3 <- ggplot(s.fires3 %>% filter(!is.na(SFC)), 
       aes(x=ISI, y=ROS)) +  
...
ggplot(s.fires.mod %>% filter(!is.na(SFC)), 
  ...
  stat_smooth(method=lm, 
              formula=y~I(x^2)-1, #same as sros.isim2.mod
              se=FALSE, colour='black') +    
...
#  geom_line(data=f.sfires, aes(x=fisim, y=fros1), colour='gray')+   
  stat_function(fun=sf.isim2sfc075.fun, color='red') +
  stat_function(fun=sf.crisim.fun, color='blue') +
  geom_text(x=28, y=sf.crisim.fun(28)-2, label='CR_ISI.sa') +
  geom_text(x=28, y=predict(sros.isim2SFC.mod, 
            newdata=list(isi.m=28, SFC=1.4))+3, 
            label='IsaSFC') +
  theme_classic()
...
#  stat_function(fun=sf.axbisi.fun, color='grey') +
 ...
 # geom_text(x=21, y=c6s(21)-1, label='C6s') +
  geom_text(x=25, y=sf.crisi.fun(25)+1, label='CR') +
...
The rate of spread, or rate of forward advance, of a wildfire is recognized as its most important descriptive characteristic for fire managers [@VanWagner1965]. Fire behaviour in the conifer forests of Canada is often a story about crown fire – high intensity, rapidly spreading fires with flames overtopping the trees and ember spotting ahead of the front [@alexanderCrownFireDynamics2016]. And yet in the chronology of fire events, most fire days feature only ground or surface fire behaviour [@Wang.etal2014]. Surface fires can be safely and routinely actioned by suppression crews and equipment [@wheatley2022], and managers need to be able to estimate the speed and intensity of fires even when crown fire activity is unlikely.

### Surface fire spread in the Canadian Fire Behavior Prediction System

Surface fire spread models have not been the primary focus of past Canadian fire behaviour modelling efforts. The present Fire Behavior Prediction System (FBPS) features fuel-type specific models encompassing the full range of surface through crown fire behaviour for a small number of conifer fuel complexes [@fcfdg1992; @tymstra2010], an evolution of concepts drafted decades earlier [@VanWagner1973a]. However, more recent and flexible conifer modelling schemes exist that are specific in terms of the type of fire predicted under given weather and moisture inputs; these include CFIS (Crown Fire Initiation and Spread), Conifer Pyrometrics, and the forthcoming 'next generation' Fire Behavior Prediction System [@Alexander2006; @perrakis2020; @cfsfiredangergroup2021]. At present, only a limited number of specialized models exist for predicting surface fire behaviour in Canadian vegetation types. For deciduous forests, open (treeless; i.e. grassland) communities, and scattered logging slash, the FBPS has appropriate fuel type models for predicting surface fire spread (i.e., the D-1, O-1 and S-1 through S-3 fuel types). Those models describe fire behaviour based on empirically-derived spread rate and fuel consumption models [@fcfdg1992]. As with the conifer fuel types, spread rate equations are based on the Chapman-Richards function [@vanwagner1989]:

$ROS=a(1-e^{-b\cdot ISI})^c$ [1],

with the a, b, and c parameters fitted for each fuel type. Newer models have also been developed for grass [@Wotton2009; @kidnie2015] and atlantic shrublands [@pepin2020]. However, the only published empirical models for predicting purely surface fire spread in conifer stands are specialized regression models derived from sparse observations at individual research sites (e.g. Lawson 1972, Quintilio 1977), data that were eventually incorporated into the FBP fuel type functions.

As the FBP System was taking shape, a more flexible and theoretical dual equilibrium conifer crown fire model concept was also proposed that included a function, RSS, representing "all possible surface fires"[@vanwagner1989]. Suggested parameters for this function appeared in the FBPS report in the form of the C-6S model, for the surface fire portion of the 'Conifer Plantation' fuel type; no explanation as to their origin: *a*=30, *b*=0.080, *c*=3 [@fcfdg1992]. Two additional RSS equations were published shortly thereafter, associated with immature and mature jack pine stands in Ontario [@vanwagner1993]. For the immature stand, the source of many observations from the C-4 fuel type [@stocks1987], we presently refer to this equation as C-4S: *a*=20, *b*=0.20, *c*=5. For the mature pine stand, associated with C-3 fuel type observations [@stocks1989], we refer to this as C-3S: *a*=15, *b*=0.05, *c*=2. Again, no evidence was presented for the origin of these parameters. Figure 1 compares these notional conifer surface fire ROS models alongside the D-1 model (*a*=30, *b*=0.0232, *c*=1.6).
...
The source data for most empirical fire models in Canada, including the FBPS, is a growing database of field-scale experimental burns conducted at various sites across Canada. These data have been previously described and analyzed [e.g., @alexander1990; @cruz1999; @perrakis2023], but never before for the purpose of examining generalized surface fire models. The present database contains over 120 conifer observations of experimental fire behaviour in stands of various sizes, but primarily 0.1-1.0 ha (median size: 0.4 ha). In addition, 32 experimental fire observations were available in deciduous stands, which includes 6 from the northeastern US. These are based on experiments described by Van Wagner [-@1973; aspen stands], Alexander and Sando (), and Quintilio et al. [-@quintilio1991].

The objective of this study was to use the database of Canadian experimental fires to produce simple and rapid models for estimating the ROS of surface fires burning in generalized conifer stands. Since fires spreading under higher danger conditions in conifer stands tend to be crown fires [@vanwagner1977; @beverly2020; @cruz2022], the majority of surface fires involve occurred during moderate-level fire danger indices and wind speeds. Such models can be used to inform dynamic fire behaviour predictions sytems, where surface fire, crown fire initiation, and crown fire spread are modelled as separate but related processes [@scott2001 ; @perrakis2020; @cfsfiredangergroup2021].

## 2. Methods

#### Fire database

The Canadian experimental fire database was the primary source of surface fire observations. As previously described [@alexander1990, @perrakis2023], varying sizes of square or rectangular plots were ignited from an exposed windward edge using line ignition techniques.

We used the surface fires in our data to fit simple empirical rate of spread (ROS) models based on the fire environment variables. Consistent with the majority of surface fire behaviour models [@sullivan2009], we focused on wind speed (WS) and dead fuel moisture content (mc) as primary variables of interest. Related weather and moisture predictors tested included open 10 m wind speed (WS~10~), estimated dead fine fuel moisture content, and the Initial Spread Index (ISI), in line with the Fire Weather Index (FWI) and FBP System conventions [@fcfdg1992; @cfsfiredangergroup2021]. Fine dead fuel moisture was estimated from the Fine Fuel Moisture Code (FFMC) component of the FWI, as well as from Wotton and Beverly's [@wotton2007] stand-adjusted moisture content (mcSA). Variables including fuel type and plot width were also examined.
...
Surface fire ROS in conifer stands ranged from `r min(fd.ros$ROS)` – `r max(fd.ros %>% filter(FiT2=='S') %>% pull(ROS))` m min^-1^, with most fires' sROS values well below the maximum (95th percentile sROS of 4.0 m min^-1^.). The linear trend shown in Fig 2 (green line) represents a very simple baseline linear surface ROS model:

$$ROS=-0.16666 + 0.19319 \cdot WS_{10}$$

with adjusted *R^2^* =0.198.

Figure 3 shows surface fire observations classified by fuel type and surface fuel consumption level. Despite identifying these fires as surface fires in primary sources, a few observations appeared anomalous, particularly those with significant estimated levels of canopy fuel consumption (CFC). Fires with estimated CFC \> 0.1 (mean CFC: 0.35 kg m\^2) were then removed from the surface model-fitting. Other questionable inclusions included fires in open Ponderosa pine / Douglas-fir stands, with partly cured understory grass [@nyberg1979; see Box 1] as well as fires observations in deciduous stands.

```{r label=Fig3}
Fig3
```

We then calculated an alternative spread index, the stand-adjusted ISI (ISI~SA~), using the same ISI formulation as originally described by Van Wagner [-@vanwagner1987], substituting the mc~SA~ value instead of the mc~FFMC~. The full equation is provided for clarity:

$$
ISI_{SA}=0.208\cdot [e^{0.05039\cdot WS_{10}}] \cdot (91.9 \cdot e^{-0.1386\cdot mc}) \cdot \left[ 1+\frac{mc^{5.31}}{(4.93\cdot 10^7)}) \right] 
$$ , [5], with *mc* representing mc~SA~. Testing various models of wind speed, ISI, and ISI~SA~ produced the following models [table: ]
...
Although the most relevant value related to wind velocity might be the mid-flame wind speed (as per [@rothermel1972]), the 10-m open wind speed is commonly measured by Canadian fire managers at several hundred weather stations across the country, and that measure was used in our models. We tested other associated forest structure variables, including canopy closure class, FSG, and ignition line exposure, in order to account for wind reduction between 10-m open conditions and flame-level (\~1 m) sub-canopy conditions.
