...
sf.isim2sfc075.2 <- function(isim) {
  a=summary(sros.isim2SFC.mod)$coefficients[1]
  b=summary(sros.isim2SFC.mod)$coefficients[2]
  sfc=0.75
  return(a*isim^2 + b*sfc)
}
#Maybe ask Perplexity how to make 'predict' work for stat_function?
...
#  geom_line(data=f.sfires, aes(x=fisim, y=fros1), colour='gray')+
  stat_function(fun=predict(sros.isim2SFC.mod), 
                            args=list(SFC=0.75), color='red') +
...
The source data for much of the FBPS, including the models described above, is a growing database of field-scale experimental burns conducted at various sites across Canada. These data have been previously described and analyzed [e.g., @Alexander.Quintilio1990; @cruz1999; @perrakis2023], but never before for the purpose of examining generalized surface fire models. The present database contains over 120 conifer observations of experimental fire behaviour in stands of various sizes, but primarily 0.1-1.0 ha (median size: 0.4 ha). In addition, 32 experimental fire observations were available in deciduous stands, based on original studies described by Van Wagner [-@VanWagner1973a; aspen stands], Alexander and Sando [@Alexander.Sando1989], and Quintilio et al. [-@quintilio1991].
...
We then calculated an alternative spread index based on the Wotton and Beverly [-@Wotton.Beverly2007] stand-adjusted moisture content estimate (mc~SA~), but in the same form as the ISI, combining wind and fuel moisture effects: the stand-adjusted ISI (ISI~SA~). The ISI~SA~ involved the same ISI formulation as originally described by Van Wagner [-@vanwagner1987], but substituting the mc~SA~ value instead of the mc~FFMC~. The full equation is provided for clarity:

$$
ISI_{SA}=0.208\cdot [e^{0.05039\cdot WS_{10}}] \cdot (91.9 \cdot e^{-0.1386\cdot mc}) \cdot \left[ 1+\frac{mc^{5.31}}{(4.93\cdot 10^7)}) \right] 
$$ , [5], with *mc* representing mc~SA~ in this case. After testing various models of wind speed, ISI, and ISI~SA~ , the following models were produced [table: ]
