...
[7]. This model had an adjusted *R^2^* of 0.459. As suggested by the negative intercept value, it predicts ROS \< 0 at low ISI values, with ROS=0 predicted at ISI of 3.64.
...
The main dataset of experimental burns conducted in Ponderosa pine-Douglas-fir forest stands in Canada was a series of experiments in the Dewdrop Range near Kamloops, British Columbia, conducted in 1978 and 1979. These seven\* experiments were originally published as student reports [@Nyberg1979;@Smaill1980], with original copies somewhat difficult to obtain. Some of the main weather and fire behaviour information was originally analyzed by members of the CFS Fire Danger Group as part of the 'C-7' fuel type, including wind speed, calculated FWI components, and measured ROS and fire type (unpublished; summarized in FCFDG 1992). However, this was the only description of the C-7 fuel type ever published, and the assumptions and calculations remained unknown. Surface fuel consumption in these experiments was reported or estimated for only one of the burn experiments, and resulted in a very high (possibly suspect) estimate of \> 5 kg m^-2^ of woody fuel consumption and total SFC of 5.3 kg m^-2^.

During the course of the present project, original copies of these documents were located and the surface fuel consumption and grass curing values identified by Nyberg and Smaill were reanalyzed. This resulted in new (much lower: 0.28 – 0.92 kg m^-2^) forest floor consumption values associated with seven of the original plots. These are open stands dominated by Ponderosa pine, with canopy closure values estimated for all plots between 23 % and 43 % (mean closure: 33.2 %), grassy understories, with grass fuel load (sum of all live and dead grasses and forbs) of 0.041 kg m^-2^. It remains unclear where the original estimate of 5.3 kg m^-2^ originated. Given the order of magnitude difference between the original value and new values reported here, some sort of error in transcription or calculation appears likely.

\*Plot numbers 1-6, burned in 1978, are described by Nyberg (1979). Plot 9, burned in 1979, is described by Smaill (1980). Plot 8 from the same site was apparently burned in 1980, but no notes or published description survives.

[end Box 1]

We tested models with various combinations of of predictors, though most model forms using more than one or two predictors were non-significant owing to the modest number of observations and high variability. The best results were obtained using combinations with *ISI* or *ISI~SA~*, as linear or squared terms. We forced the intercept through zero due to the known lack of fire spread potential during high moisture conditions (e.g. ISI \< 2).

Table 1 shows some the parameters and evaluation metrics of some of the more useful models along with the previously described FBP-era models. As the table indicates, some of the models were fitted to the entire dataset of surface fire observations, with or without a surface fuel consumption variable.

The 'aggregated data' models used a dataset that included all observations except those with canopy fuel involvement, as noted previously; the PPDF fires were adjusted to a curing rate of 90%, as per Cheney##. Other models used only the boreal and sub-boreal conifer observations to avoid influence from either the deciduous experiments or the PPDF fires.
...
sfc.isivals <- mapply(FUN=sf.isi2sfc.fun, isi=rep(isi.vals, 3), 
  ...
  matrix(nrow=3, ncol=3, byrow=TRUE) %>%  #calculate ROS for each combo
  as.data.frame() 
colnames(sfc.isivals) <- c('ISI.5', 'ISI.10', 'ISI.15')

sfc.isimvals <- mapply(FUN=sf.isim2sfc.fun, isim=rep(isi.vals, 3),
                sfc=rep(sfc.levels, each=3)) %>%
