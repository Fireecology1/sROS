...
df.m25 <- fd.ros %>% filter(FiT2=='S') %>% 
  select(ws, ROS) %>% 
  mutate(pred25 = 0.25 * ws,
         manual.resid = ROS-pred25)
...
df.line.m25 <- tibble(num=15, Model='25 perc', 
                          nobs=NA, RMSE=NA,
         MAE=mae.manual(actual=df.m25$ROS, pred=df.m25$pred25),
         MAPE=mape.manual(actual=df.m25$ROS, pred=df.m25$pred25),
         EfronR2=ef.r.squared(actual=df.m25$ROS, 
                              pred=df.m25$pred25)
  )
...
Table 1 shows the predictors and evaluation metrics for the list of fitted models, as well as *ROS* predictions at three levels (5, 10, 15) of *ISI* or *ISI~sa~*. Other variables (FFMC, SFC) were assigned constant values where needed for calculation purposes, as noted. These models can be also be compared with predictions from the previously described Van Wagner and FBP surface fire models (Table 1). As the table indicates, the datasets used varied from `r min(acc.tab4$nobs, na.rm=T)` to `r max(acc.tab4$nobs, na.rm=T)` observations. Models forms 1–3 and 12–13 used only the boreal conifer (pine and spruce surface fire) observations, and had slightly better performance (higher Efron's r-squared, lower MAE and MAPE) than models fitted to the larger aggregated dataset (Models 4 – 11). For the two models that include *sqrt(SFC)* as a covariate, Table 2 shows predictions using other SFC values; the SFC influence is greatest at low ISI or ISI~sa~ levels, and is minimal by ISI 15. Non-linear model forms using the sigmoidal Chapman-Richards form had insufficient data ('nls' function failed to converge) to automatically fit the *a* parameter; it was set to 20 following the assumed value for Van Wagner's C4s model (Van Wagner 1993).

The 'aggregated data' models included all surface fire observations except those with canopy fuel involvement, as noted previously. This includes fires in deciduous stands of aspen (29 observations), oak (2), or mixedwood (1) as well as seven PPDF fires. Six deciduous fires took place in the northeastern US, the remainder in Canada. Boreal conifer models used only fires in jack pine, black spruce, jack pine/black spruce, red pine, lodgepole pine, and red pine/white pine [@perrakis2023].
